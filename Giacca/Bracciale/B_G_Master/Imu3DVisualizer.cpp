...
bool Imu3DVisualizer::begin() {
  ...
  // gigsDisplay.setRotation(90);      //impostazione orientamento Schermo
  ...
  gigaDisplay.setTextColor(BIANCO);
  gigaDisplay.setCursor(10, 380);
  gigaDisplay.print("RAW ACC:");
  gigaDisplay.setCursor(10, 420);
...
void Imu3DVisualizer::drawSphere() {
  int cx = gigaDisplay.width() / 2;
  int cy = gigaDisplay.height() / 2;
  int r = 100;
  ...
  if (millis() - lastTxt > 100) {
    ...
    // Clear value areas
    gigaDisplay.fillRect(120, 380, 600, 80, NERO);

    gigaDisplay.setTextSize(2);

    // ROW 1: RAW ACCELEROMETER
    gigaDisplay.setCursor(120, 20);
    if (accSuccess) gigaDisplay.setTextColor(BIANCO);
    else gigaDisplay.setTextColor(ROSSO);  // Red if read failed
    ...
    gigaDisplay.setCursor(120, 50);
    gigaDisplay.setTextColor(CIANO);
    gigaDisplay.print("Roll:");
    gigaDisplay.print(roll, 1);

    gigaDisplay.setCursor(120, 70);
    gigaDisplay.setTextColor(CIANO);
    gigaDisplay.print(" Pitch:");
    gigaDisplay.print(pitch, 1);

    gigaDisplay.setCursor(120, 90);
    gigaDisplay.setTextColor(CIANO);
    gigaDisplay.print(" Yaw:");
    gigaDisplay.print(yaw, 1);
  }
}
